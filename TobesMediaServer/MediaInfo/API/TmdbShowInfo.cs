...
{
    public class TmdbShowInfo : IShowInfo
    {
        protected virtual bool ShowAnime { get => false; }
        HttpClient m_client = new HttpClient();
        private const string m_basePosterURL = "https://image.tmdb.org/t/p/original";

        private IUsenetIndexer m_indexer;
        private ThetvdbShowInfo m_tvdbInfo;

        public TmdbShowInfo(IUsenetIndexer indexer)
        {
            m_indexer = indexer;
            m_tvdbInfo = new ThetvdbShowInfo(indexer);
        }
        ...
        private async Task<MediaBase> ParseMediaFromJsonAsync(JObject json, bool checkDownload, List<MediaBase> mediaList = null)
        {
            ...
            if (!isAnime.HasValue)
                return null;
            if (isAnime.Value != ShowAnime)
                return null;
            if (checkDownload && await m_indexer.GetShowLinkByNzbIdAsync(tvdbid) == string.Empty)
            {
                return null;
            }
            ...
            if (posterPath == null || posterPath == string.Empty)
                return null;
            MediaBase media = new MediaBase(json["name"].ToString(), "", m_basePosterURL + json["poster_path"].ToString(), tvdbid);
            if (mediaList != null)
                mediaList.Add(media);
            return media;
        }
